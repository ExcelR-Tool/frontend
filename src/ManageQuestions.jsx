...
const ManageQuestions = () => {
  ...
  const fetchCompanies = async () => {
    try {
      ...
      const res = await axios.get('https://backend-production-6281.up.railway.app/api/companies', {
  ...
  const fetchDepartments = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('https://backend-production-6281.up.railway.app/api/departments', {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const fetchYears = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('https://backend-production-6281.up.railway.app/api/years', {
        headers: { Authorization: `Bearer ${token}` },
      });
      setYears(res.data);
    } catch (err) {
  ...
  const fetchSections = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('https://backend-production-6281.up.railway.app/api/sections', {
        headers: { Authorization: `Bearer ${token}` },
      });
...
<div style={{ margin: '20px auto', padding: '20px', maxWidth: '700px', border: '1px solid #ccc', borderRadius: '8px' }}>
  ...
  <form onSubmit={handleAddQuestion} style={{ display: 'flex', flexDirection: 'column', gap: '15px', backgroundColor: '#f9f9f9', padding: '25px', borderRadius: '10px', boxShadow: '0 0 10px rgba(0,0,0,0.1)' }}>
    ...
    <select name="yearId" value={formData.yearId} onChange={handleInputChange} required>
  ...
  {years.map((year) => (
    ...
    <select name="sectionId" value={formData.sectionId} onChange={handleInputChange} required>
  ...
  {sections.map((section) => (
    ...
    <input name="subject" placeholder="Subject" onChange={handleInputChange} required />
    <input name="question" placeholder="Question" onChange={handleInputChange} required />

    {formData.options.map((opt, i) => (
      <input key={i} name={`option${i}`} placeholder={`Option ${i + 1}`} value={opt} onChange={handleInputChange} required />
    ))}

    <input type="number" name="correctAnswerIndex" placeholder="Correct Option Index (0-3)" min="0" max="3" value={formData.correctAnswerIndex} onChange={handleInputChange} required />

    <button type="submit" style={{ backgroundColor: '#1976d2', color: '#fff', padding: '12px', fontSize: '1rem', border: 'none', borderRadius: '6px', cursor: 'pointer' }}>➕ Add Question</button>
  </form>
